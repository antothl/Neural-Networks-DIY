...
class Linear(Module):
    ...
    def forward(self, X):
        X = np.hstack([X, np.ones((X.shape[0], 1))])  #colonne de biais
        return X @ self._parameters  

    def backward_update_gradient(self, input, delta):
        input = np.hstack([input, np.ones((input.shape[0], 1))])  # Ajout du biais
        
        # Debugging: Vérification des dimensions
        print(f"input.T shape: {input.T.shape}, delta shape: {delta.shape}")
        
        # Correction: s'assurer que delta a bien la bonne forme
        if delta.ndim == 1:
...
class TanH(Module):
    ...
    def backward_delta(self, X, delta):
        return delta * (1 - np.tanh(X) ** 2)  # Dérivée correcte de tanh(x)
...
class Sigmoide(Module):
    ...
    def backward_delta(self, X, delta):
        sig = 1 / (1 + np.exp(-X))
        return delta * sig * (1 - sig)  # Dérivée correcte de la sigmoïde
...
class SimpleNN(Module):
    ...
    def backward(self, X, y, loss):
        ...
        delta2 = self.sigmoid.backward_delta(self.z2, delta)  # Correcte propagation de gradient
        delta1 = self.linear2.backward_delta(self.a1, delta2)
        delta0 = self.tanh.backward_delta(self.z1, delta1)  # Correcte propagation de gradient
...
class Sequentiel(Module):
    ...
    def forward(self, X):
        self.inputs = [X]  # Stocker les entrées pour backward
        for module in self.modules:
            X = module.forward(X)
            self.inputs.append(X)  # Stocker chaque sortie
        return X
    ...
    def backward(self, X, y, loss):
        ...
        for i in range(len(self.modules) - 1, -1, -1):  # Backpropagation
            delta = self.modules[i].backward_delta(self.inputs[i], delta)
            self.modules[i].backward_update_gradient(self.inputs[i], delta)
...
class Optim:
    ...
    def step(self, batch_x, batch_y):
        ...
        return self.loss.forward(batch_y, self.net.forward(batch_x)).mean()